    ...
    <div>
      <div className="texture-selector icon-container">
        {Object.entries(images).map(([k, src], index) => {
          return (
            <div key={k}>
              <span className="absolute text-[10px] m-1 translate-x-[6px] translate-y-[6px] text-[#000000cd] font-semibold">
                {index + 1}
              </span>
              <img
                key={k}
      ...
      <div className="icon-container absolute right-0 top-[30%] z-10 cursor-pointer">
        <div onClick={() => setSettingMenu(!settingMenu)}>
          <span className="absolute text-[9px] m-1 translate-x-[7px] translate-y-[3px] text-[#000000cd] font-semibold">
            E
          </span>
          <img
            src={gearIcon}
            className={`${
              "setting" === activeConfig ? "active" : ""
            } bg-[#FBBA4A]`}
          />
        </div>
        ...
        <div onClick={() => setInventoryBar(!inventoryBar)}>
          ...
          <img
            src={bagPackIcon}
            className={`${
              "inventory" === activeConfig ? "active" : ""
            } bg-[#19B294]`}
          />
        </div>
        <div onClick={() => setInfoBar(!infoBar)}>
          <span className="absolute text-[9px] m-1 translate-x-[7px] translate-y-[3px] text-[#000000cd]">
            I
          </span>
          <img
            src={InfoIcon}
            className={`${
              "infoMenu" === activeConfig ? "active" : ""
            } bg-[#5789C4]`}
          />
          {infoBar && (
            <ul className="btn absolute font-sans -translate-x-[250px] flex-col -translate-y-[75px] w-[250px] text-[18px] py-2 px-3 font-vt">
